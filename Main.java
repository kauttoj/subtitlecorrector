...
public class Main {
	...
	JLanguageTool langTool1,langTool2;
...
*/
	...
	public void doProofRead(GUI myGUI) {
		/*
		...
		try {
			...
			for (SingleLine line : data) {
				...
				if (!matches1.isEmpty() && !matches2.isEmpty()) {			
					...
					for (int j=0;j<lista3.size() && j<20;j++) {						
						...
						if ( ss.length()>1 && Character.isUpperCase(ss.charAt(0)) && Character.isLowerCase(ss.charAt(1)))
							continue;
						if ( ss.length()>1 && ss.substring(ss.length() - 1)=="'" )
							continue;						
						k++;
						if (k==1)
							myGUI.printConsole("  subtitle " + i + ":");
							//System.out.print("subtitle " + i + ":\n    "); //  match.getMessage()
						//else if (k>1)
						//	myGUI.printConsole(", "); //  match.getMessage()
						myGUI.printConsole("      " + ss);// + match.getSuggestedReplacements()); //  match.getMessage()
						//System.out.print(" [ " +
						//		match.getSuggestedReplacements() + " ]\n");
					}			
					//if (k>0) {
